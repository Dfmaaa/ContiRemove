...
public class ContiRemove{
 ...
 static File[] makearray(String[] strarray){
  File[] a=new File[strarray.length];
  for(int x=0;x<strarray.length;x++){
   a[x]=new File(strarray[x]);
  }
  return a;
 }
 public static void interpreter(File a) throws IOException, FileNotFoundException{
   String source=getfileline(a, 0).split(" ")[0];
   String target=getfileline(a,0).split(" ")[1];
  File[] marray=makearray(getfileline(a,1).split(","));
  File[] tarray=new File(target).listFiles();
  if(compare(marray,new File(target).listFiles())==false){
    System.out.println("Lengths should be same.");
  }
  else if(compare(marray,new File(target).listFiles())){
   ...
   //real code
    ...
    if(schoice.equals("T")){
      ...
      for(int x=0;x<marray.length;x++){
       ...
       if(os.equals("Windows")){
       marray[x]=new File(path+"\\"+input.nextLine());
       }
       else if(os.equals("Linux")){
        marray[x]=new File(path+"/"+input.nextLine());
       }
